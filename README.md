...
### 3. 🔀 多维度自定义排序与持久化 (新特性)
- **多维度排序规则**：支持按 **名称**（智能数字自然排序，如第1话、第2话、第10话）、**修改时间**（最后修改日期）、**创建时间**（文件诞生日期）、**文件大小**（体积）自由排列作品与目录。
- **升序 / 降序一键切换**：支持升序（A ➔ Z / 旧 ➔ 新 / 小 ➔ 大）与降序（Z ➔ A / 新 ➔ 旧 / 大 ➔ 小）。
- **多端响应与状态持久化**：桌面端顶部工具栏、移动端侧滑抽屉与设置中心均可即时调节并自动保存用户偏好。

### 4. 🌓 暗色 / 亮色主题切换与跟随系统 (新特性)
- **三档外观模式自由选择**：支持 **暗色模式 (Dark)**、**亮色模式 (Light)** 与 **跟随系统 (Auto)**。
- **智能跟随系统外观**：选择「跟随系统」档位后，网站通过原生媒体查询监听设备外观变化，当系统切换深浅色时自动无缝适配。
- **极致防闪烁体验 (Anti-FOUC)**：页面首帧加载前即时识别持久化偏好并设定 `color-scheme`，杜绝暗色亮色切换时的白屏闪烁。
- **全端快捷切换**：桌面端顶部工具栏配有一键循环切换按钮，移动端侧滑抽屉及「设置」中心均配备直观的三宫格主题选择面板。
